...
const { registerSchema, loginSchema, forgotPasswordSchema } = require('../validation/userSchema');
...
exports.forgotPassword = async (req, res, next) => {
  try {
    ...
    res.status(200).json({
      success: true,
      message: 'If your email exists in our system, you will receive a password reset link',
      // Only for demonstration - in production don't send the token in response
      resetToken
    });
  } catch (error) {
    next(error);
...
// Google authentication route has been removed
